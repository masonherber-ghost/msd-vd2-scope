...
- Any Figma values with no match in `src/globals.css` (`@theme`)
- Interaction and hover states not visible in static frames
- Responsive behaviour at each breakpoint — base (mobile), then the four `--breakpoint-*` tokens in `src/globals.css`
